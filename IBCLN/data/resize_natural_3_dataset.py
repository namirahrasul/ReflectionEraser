...
from data.image_folder import make_dataset, make_singleImage_dataset
...
class ResizeNatural3Dataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if not opt.phase == 'train':
            if opt.is_single_image:
                self.B_paths = sorted(make_singleImage_dataset(opt.single_image_path))
                # print(self.B_paths)
            else:
                self.B_paths = sorted(make_dataset(self.dir_B, opt.max_dataset_size))  # load images from '/path/to/data/trainB'
            # print("ssssssssFileNames", self.B_paths)
