...
class RealDataset(BaseDataset):
    def __init__(self, datadir, fns=None, size=None, single_image_path = None):
        super(RealDataset, self).__init__()
        self.size = size
        self.datadir = datadir
        self.fns = []
        self.fns.append(os.path.basename(single_image_path))
        # print(single_image_path, self.fns)
        # or os.listdir(join(datadir))

        # if single_image_path is not None:
        #     self.fns = []
        #     self.fns.append(single_image_path)
    ...
    def __getitem__(self, index):
        ...
        max_size = 300
        m_img = Image.open(self.datadir).convert('RGB')
        m_img.thumbnail((max_size, max_size))
